...
game_over = 0
Lucynamelist = [
  'Peggy Pyre', 'Padded Pyre', 'Flat Brat', 'Tiny', 'Sally Pyre', 'Pissy Pyre',
  'Stinky Pyre', 'Juicy Lucy', 'Titsie Pyre', 'Blue M&M', 'Cummander Pyre',
  'White-haired Numi', 'Lucy PooPoo', 'Delululucy', 'Legussy Pyre'
]
hurtlist = [6, 7]
...
#potion image
potion_img = pygame.image.load('img/icon/Sippieicon2.png').convert_alpha()
restart_img = pygame.image.load('img/icon/restartbutton2.png').convert_alpha()
restart_img = pygame.transform.scale(
  restart_img, (restart_img.get_width() * 3, restart_img.get_height() * 3))
...
defeat_img = pygame.transform.scale(
  defeat_img, (defeat_img.get_width() * 2, defeat_img.get_height() * 2))
#mouse image
sword_img = pygame.image.load('img/icon/hammer.png').convert_alpha()
sword_img = pygame.transform.scale(
  sword_img, (sword_img.get_width() * 0.3, sword_img.get_height() * 0.3))
...
def draw_panel():
  ...
  #show player stats
  draw_text(f'{ratJAM1.name}', font, white, 100,
            screen_height - bottom_panel + 39)
  draw_text(f'HP: {ratJAM1.hp}', font, white, 295,
            screen_height - bottom_panel + 39)
  for count, i in enumerate(wiggly_list):
    #show name and health of wigglie
    draw_text(f'{i.name} HP: {i.hp}', font, white, 440 + (count * 185),
              (screen_height - bottom_panel + 39))
...
class ratJAM:
  ...
  def attack(self, target):
    ...
    if target.hp < 1:
      ...
      target.alive = False
      target.death()
    damage_text = DamageText(target.rect.centerx, target.rect.y, str(damage),
                             red)
...
class Demon():
  ...
  def __init__(self, x, y, name, max_hp, strength, potions):
    ...
    self.animation_list = []
    self.frame_index = 0
    self.action = 0  #0:idle, 1:intro, 2:attack, 3:attack v2, 4: attack v3 5: attack v4, 6: Cry, 7: Hurty(removed for web version), 7: Baby, 9: RIPBOZO(removed for web version), 8: Victory
    self.update_time = pygame.time.get_ticks()
    #load idle images
    temp_list = []
    for i in range(106):
      ...
      img = pygame.transform.scale(
        img, (img.get_width() * 0.8, img.get_height() * 0.8))
      temp_list.append(img)
    self.animation_list.append(temp_list)
    #load intro images
    temp_list = []
    ...
    for i in range(76):
      ...
      img = pygame.transform.scale(
        img, (img.get_width() * 1.1, img.get_height() * 1.1))
      temp_list.append(img)
    self.animation_list.append(temp_list)
    ...
    for i in range(159):
      ...
      img = pygame.transform.scale(
        img, (img.get_width() * 0.6, img.get_height() * 0.6))
      temp_list.append(img)
    self.animation_list.append(temp_list)
    #load Hurty images
#    temp_list = []
...
#    self.animation_list.append(temp_list)
    ...
    for i in range(133):
      ...
      img = pygame.transform.scale(
        img, (img.get_width() * 0.93, img.get_height() * 0.93))
      temp_list.append(img)
    self.animation_list.append(temp_list)
    #load RIPBOZO images
 #   temp_list = []
...
#   self.animation_list.append(temp_list)
  ...
  def hurt(self):
    ...
    randhurt = random.choice(hurtlist)
...
class wiggly():
  ...
  def __init__(self, x, y, name, max_hp, strength, potions):
    ...
    for i in range(22):
      ...
      img = pygame.transform.scale(
        img, (img.get_width() * 0.2, img.get_height() * 0.2))
      temp_list.append(img)
    self.animation_list.append(temp_list)
    #load attack images
    temp_list = []
...
class Special():
  ...
  def __init__(self, x, y):
    ...
    self.action = 0  #0: OneGuy, 1: Sussy(removed for web version), 2: Corpa(removed for web version), 3:Marriage, 4:Moderating, 5:Gayge
    self.update_time = pygame.time.get_ticks()
    #load OneGuy images
    temp_list = []
    for i in range(142):
...
ratJAM_health_bar = HealthBar(100, screen_height - bottom_panel + 60,
                              ratJAM1.hp, ratJAM1.max_hp)
wiggly1_health_bar = wigglyHealthBar(435, screen_height - bottom_panel + 60,
                                     wiggly1.hp, wiggly1.max_hp)
wiggly2_health_bar = wigglyHealthBar(620, screen_height - bottom_panel + 60,
                                     wiggly2.hp, wiggly2.max_hp)
Lucy_health_bar = BossHealthBar(450, screen_height - bottom_panel + 60,
                                LucyPyre.hp, LucyPyre.max_hp)

#create buttons
potion_button = Button(screen, 23, screen_height - bottom_panel + 40,
                       potion_img, 64, 64)
...
while run:
  ...
  draw_text('Potions: ' + str(ratJAM1.potions), font, white, 100,
        ...
        if action_cooldown >= action_wait_time:
          ...
          if potion == True:
            if ratJAM1.potions > 0:
              ...
              ratJAM1.potions -= 1
              damage_text = DamageText(ratJAM1.rect.centerx, ratJAM1.rect.y,
                                       str(heal_ammount), green)
              damage_text_group.add(damage_text)
              current_fighter += 1
              action_cooldown = 0
        ...
        if wiggly.alive == True:
          ...
          if action_cooldown >= action_wait_time:
            ...
            if (wiggly.hp / wiggly.max_hp) < 0.5 and wiggly.potions > 0:
              ...
              wiggly.potions -= 1
              damage_text = DamageText(wiggly.rect.centerx, wiggly.rect.y,
                                       str(heal_ammount), green)
              damage_text_group.add(damage_text)
              current_fighter += 1
              action_cooldown = 0
  ...
  if alive_wigglys == 0:
    ...
    draw_text(str(randname), font, white, 450,
              (screen_height - bottom_panel) + 39)
    draw_text(f'HP: {LucyPyre.hp}', font, white, 700,
              (screen_height - bottom_panel) + 39)
